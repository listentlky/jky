...
public class PDFViewController implements OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    private final String TAG="leon";

    private ImageView btn_prev = null;
...
//            btn_redo.setVisibility(View.GONE);
        ...
        if (mNavigationMode == NAVIGATION_THUMBS) {
            ...
            m_bar_seek = new PDFBotBar(m_parent, R.layout.bar_floor_drawing_bottom);
            layout = (RelativeLayout) m_bar_seek.BarGetView();
            btn_prev = (ImageView) layout.findViewById(R.id.btn_prev);
            btn_next = (ImageView) layout.findViewById(R.id.btn_next);
            btn_undo = (ImageView) layout.findViewById(R.id.btn_undo);
            btn_redo = (ImageView) layout.findViewById(R.id.btn_redo);
            btn_undo.setColorFilter(Global.toolbar_icon_color);
            btn_redo.setColorFilter(Global.toolbar_icon_color);
...
//        btn_redo.setVisibility(RadaeePDFManager.sHideRedoButton ? View.GONE : View.VISIBLE);
    ...
    public void SetBtnChecked(ImageView btn, boolean check) {
        if (check) {
    ...
    public void OnBlankTapped() {
        Log.i(TAG,"PDFViewController OnBlankTapped m_bar_status="+m_bar_status+", mNavigationMode="+mNavigationMode);
        switch (m_bar_status) {
            case BAR_NONE:
                m_bar_cmd.BarShow();
...
//            mThumbView.thumbGotoPage(pageno);
    ...
    public void onPageModified(int pageno) {
        Log.i("leon", "PDFViewController onPageModified");
    ...
    public void onClick(View arg0) {
        ...
        if (arg0 == btn_view)//popup view list
        {
            ...
            Log.i("leon","PDFViewController btn_annot_note clicked m_set=" + m_set);
            if (m_set) {
                m_view.PDFSetNote(1);
                SetBtnEnabled(btn_annot_polygon, m_view.PDFCanSave());
                SetBtnEnabled(btn_annot_polyline, m_view.PDFCanSave());
            ...
            m_view.PDFSetView(6);
            m_menu_view.MenuDismiss();
        }else if (arg0 == btn_next) {
            Log.i("leon","PDFViewController btn_next pressed");
            m_view.onButtonNextClicked();
            m_bar_seek.BarHide();

        }else if (arg0 == btn_prev) {
            Log.i("leon","PDFViewController btn_prev pressed");
            m_view.onButtonPrevClicked();
            m_bar_seek.BarHide();
        }
    }
    ...
    public void savePDF() {
        Log.d("bdd","m_view.PDFSave(): "+m_view.PDFSave());
        if (m_view.PDFSave()) {
