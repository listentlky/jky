...
class ProjectCreateTypePresenter : BasePresenter(), IProjectContrast.IProjectCreateTypePresenter,
        ...
        if (num == 0) {
            ...
            if (mAboveOldIndex == 0 && above.size < 1) {
                for (i in 0 until num) {
                    ...
                    var buildingFloorBean = BuildingFloorBean(name + "层", arrayListOf(), "地上")
    ...
    override fun addAfterFlourList(num: Int) {
        ...
        if (num == 0) {
            ...
            if (mBeforeOldIndex == 0 && before.size < 1) {
                for (i in 0 until num) {
                    var name = NumberUtil.num2Chinese(i + 1)
                    var buildingFloorBean = BuildingFloorBean("负"+name + "层", arrayListOf(), "地下")
    ...
    fun createLocalBuilding(
        activity: Activity, mLocalProjectId: Int,
        mBuildingId: Long, name: String, leader: String, inspector: String
    ): Long? {
        ...
        mCurDrawingsDir =
            "/" + ModuleHelper.DRAWING_CACHE_FOLDER + "/" + mProName + "/" + name + "/"
